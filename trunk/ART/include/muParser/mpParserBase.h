...
//Sadjad
//auto_ptr::reset() is not implemented in VC++ 6 (and maybe other versions). If the compiler is VC++ 6 or earlier,
//we'll include a custom memory.h, where auto_ptr contains reset().
...
MUP_NAMESPACE_START
  ...
  */
  ...
  private:
    ...
    void ApplyBinAndTernaryOprt(Stack<ptr_tok_type> &a_stOpt,
    ...
    void  ClearExpr();
    void  CheckArgTypes(const ptr_val_type *pArg, int iNumArg, ICallback *pFunc) const;
    ...
    */
    ...
    bool m_bEnableOptimization;

    mutable RPN m_rpn;                  ///< reverse polish notation
    mutable val_vec_type m_vStackBuffer;
    mutable ptr_val_type m_BufferValue; ///< A value used for buffering the string parsing result
