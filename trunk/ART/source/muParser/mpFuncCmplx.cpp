...
//Sadjad
	...
	//two functions strangely not defined...
	inline std::complex<double> tan(const std::complex<double>& x)
	{
		return std::sin(x) / std::cos(x);
	}

	inline std::complex<double> tanh(const std::complex<double>& x)
	{
		return std::sinh(x) / std::cosh(x);
	}

	//It seems in VC++ 6 the complex version of the following functions are defined in the std namespace
	using std::cos;
	using std::cosh;
	using std::sin;
	using std::sinh;
	...
	//but we also need the "normal" cos, sin, ..., so we have to state them here (as members of the global
	//namespace, so that the compiler finds both...
	using ::cos;
	using ::sin;
	using ::sqrt;
	using ::log;
  ...
  FunCmplxReal::FunCmplxReal(IPackage *package)
    :ICallback(cmFUNC, _T("real"), _T("f:c"), package)
  {}
  ...
  FunCmplxImag::FunCmplxImag()
    :ICallback(cmFUNC, _T("imag"), _T("f:c"))
  {}
  ...
  FunCmplxConj::FunCmplxConj()
    :ICallback(cmFUNC, _T("conj"), _T("c:c"))
  {}
  ...
  FunCmplxArg::FunCmplxArg()
    :ICallback(cmFUNC, _T("arg"), _T("f:c"))
  {}
  ...
  FunCmplxNorm::FunCmplxNorm()
    :ICallback(cmFUNC, _T("norm"), _T("f:c"))
  {}
  ...
  FunCmplxCos::FunCmplxCos()
    :ICallback(cmFUNC, _T("cos"), _T("c:c"))
  {}
  ...
  FunCmplxSin::FunCmplxSin()
    :ICallback(cmFUNC, _T("sin"), _T("c:c"))
  {}
  ...
  FunCmplxCosH::FunCmplxCosH()
    :ICallback(cmFUNC, _T("cosh"), _T("c:c"))
  {}
  ...
  FunCmplxSinH::FunCmplxSinH()
    :ICallback(cmFUNC, _T("sinh"), _T("c:c"))
  {}
  ...
  FunCmplxTan::FunCmplxTan()
    :ICallback(cmFUNC, _T("tan"), _T("c:c"))
  {}
  ...
  FunCmplxTanH::FunCmplxTanH()
    :ICallback(cmFUNC, _T("tanh"), _T("c:c"))
  {}
  ...
  FunCmplxSqrt::FunCmplxSqrt()
    :ICallback(cmFUNC, _T("sqrt"), _T("c:c"))
  {}
  ...
  FunCmplxExp::FunCmplxExp()
    :ICallback(cmFUNC, _T("exp"), _T("c:c"))
  {}
  ...
  FunCmplxLog::FunCmplxLog()
    :ICallback(cmFUNC, _T("log"), _T("c:c"))
  {}
  ...
  FunCmplxLog10::FunCmplxLog10()
    :ICallback(cmFUNC, _T("log10"), _T("c:c"))
  {}
  ...
  FunCmplxLog2::FunCmplxLog2()
    :ICallback(cmFUNC, _T("log2"), _T("c:c"))
  {}

  //-----------------------------------------------------------------------
  void FunCmplxLog2::Eval(ptr_val_type &ret, const ptr_val_type *a_pArg, int)
  {
    //war: std::complex<float_type> v(a_pArg[0]->GetFloat(), a_pArg[0]->GetImag());
    std::complex<float_type> v = std::complex<float_type>(a_pArg[0]->GetFloat(), a_pArg[0]->GetImag());
  ...
  FunCmplxAbs::FunCmplxAbs()
    :ICallback(cmFUNC, _T("abs"), _T("f:c"))
  {}
