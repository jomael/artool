...
MUP_NAMESPACE_START
  ...
  FunParserID::FunParserID()
    :ICallback(cmFUNC, _T("parserid"), _T("s:v"))
  {}
  ...
  FunMax::FunMax() : ICallback(cmFUNC, _T("max"), _T("f:*"))
  {}
  ...
  void FunMax::Eval(ptr_val_type &ret, const ptr_val_type *a_pArg, int a_iArgc)
  {
    ...
    for (int i=0; i<a_iArgc; ++i)
    {
      ...
      case 'c':
        if (a_pArg[i]->GetImag() == 0)
        {
          val = a_pArg[i]->GetFloat();
          break;
        }
//        else
//        {
//          std::cerr << "HELP:" << a_pArg[i]->GetImag() << std::endl;
//        }
      default:
        {
          ErrorContext err;
  ...
  FunMin::FunMin() : ICallback(cmFUNC, _T("min"), _T("f:*"))
  {}
...
//    std::cerr << "END MIN" << std::endl;
  ...
  */
  ...
  FunSizeOf::FunSizeOf()
    :ICallback(cmFUNC, _T("sizeof"), _T("i:a"))
  {}
