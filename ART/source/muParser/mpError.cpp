...
MUP_NAMESPACE_START
  ...
  */
    ...
    :m_vErrMsg(0)
  {
    ...
    m_vErrMsg[ecTYPE_CONFLICT_FUN]       = _T("Argument $ARG$ of function/operator \"$IDENT$\" is of type '$TYPE1$' whereas type '$TYPE2$' is expected.");
    m_vErrMsg[ecTYPE_CONFLICT_IDX]       = _T("Index must be a positive integer value");
    ...
    m_vErrMsg[ecUNEXPECTED_SQR_BRACKET]  = _T("Unexpected square bracket.");
    ...
    m_vErrMsg[ecINDEX_OUT_OF_BOUNDS]     = _T("Index out of bounds");
    m_vErrMsg[ecASSIGNEMENT_TO_VALUE]    = _T("Assignment operator \"$IDENT$\" can not be applied to value items.");
    ...
    ,m_ErrMsg(ParserErrorMsg::Instance())
  {
    ...
    //ReplaceSubString(m_sMsg, "$EXPR$",  a_Err.Expr);
    //ReplaceSubString(m_sMsg, "$IDENT$", a_Err.Ident);
    //ReplaceSubString(m_sMsg, "$POS$",   a_Err.Pos);
    //ReplaceSubString(m_sMsg, "$ARG$",   a_Err.Arg);
    //ReplaceSubString(m_sMsg, "$TYPE1$", a_Err.Type1);
    //ReplaceSubString(m_sMsg, "$TYPE2$", a_Err.Type2);
  }
